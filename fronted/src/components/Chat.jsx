...
function Chat() {
  ...
  useEffect(() => {
    setMessages([{ sender: "bot", text: "👋 Hello! I’m 🤖 MyRKDF Saarthi — your smart university assistant. How can I help you today?" }]);
  }, []);
  ...
  return (
    <div className="flex flex-col h-screen text-white" style={{
      backgroundImage: `
      ...
      <div className="p-4 text-center border-b border-white/10 backdrop-blur-md bg-black/30">
        <h2 className="text-3xl md:text-4xl font-extrabold flex items-center justify-center gap-2">
          <span>🤖</span>
          <span className="bg-gradient-to-r from-red-400 via-yellow-400 to-orange-500 text-transparent bg-clip-text">MyRKDF Saarthi</span>
          <span style={{ color: "initial" }}>🎓</span>
        </h2>
        <p className="text-sm text-gray-300 mt-1 tracking-wide">✨ RKDF University • AI Assistant</p>
      </div>
      ...
      <div className="p-4 bg-black/40 backdrop-blur-xl border-t border-white/10 flex gap-3">
        <input
          type="text"
          className="flex-1 px-5 py-3 rounded-full bg-white/10 backdrop-blur-xl text-white placeholder-gray-300 outline-none border border-orange-400/30 focus:ring-2 focus:ring-yellow-400/70 focus:border-yellow-400/60 transition-all"
        ...
        <button
          onClick={handleSend}
          disabled={isTyping}
          className="px-6 rounded-full font-semibold text-black bg-gradient-to-r from-cyan-400 to-blue-500 hover:scale-105 transition shadow-[0_0_25px_rgba(59,130,246,0.6)] disabled:opacity-50"
        >
          🚀 Send
        </button>
      </div>
